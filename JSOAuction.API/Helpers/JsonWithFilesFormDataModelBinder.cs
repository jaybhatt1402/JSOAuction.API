...
namespace JSOAuction.API.Helpers
{
    public class JsonWithFilesFormDataModelBinder : IModelBinder
    {
        ...
        public async Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            if (bindingContext.HttpContext.Request.Form.TryGetValue("htmlEmail", out StringValues htmlValue))
            {
                var htmlString = htmlValue.ToString();
                var htmlStringProperty = bindingContext.ModelMetadata.Properties.SingleOrDefault(p => p.PropertyName == "HtmlEmail");
                if (htmlStringProperty != null)
                {
                    htmlStringProperty.PropertySetter(model, htmlString);
                }
            }

            if (bindingContext.HttpContext.Request.Form.TryGetValue("htmlToRender", out StringValues htmlValues))
            {
                var htmlString = htmlValues.ToString();
                var htmlStringProperty = bindingContext.ModelMetadata.Properties.SingleOrDefault(p => p.PropertyName == "HtmlToRender");
                if (htmlStringProperty != null)
                {
                    htmlStringProperty.PropertySetter(model, htmlString);
                }
