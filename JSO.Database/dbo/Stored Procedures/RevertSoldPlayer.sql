...
-- =============================================
CREATE PROCEDURE [dbo].[RevertSoldPlayer]
(
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SET PlayerStatus = 'disclosed', TeamId = NULL, ShowSoldPopup = NULL
	...
	IF EXISTS (SELECT 1 FROM Bids WHERE PlayerId = @PlayerId AND AuctionId = @AuctionId AND IsDeleted = 0 AND Sold = 1)
    BEGIN
        --UPDATE Bids 
        --SET Sold = 1 
		...
		SET Sold = 0
		WHERE BidId = (
					SELECT TOP 1 BidId from bids WHERE AuctionId = @AuctionId AND PlayerId = @PlayerId AND Sold = 1
